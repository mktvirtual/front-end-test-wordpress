...
function the_posts_navigation() {
	...
	<nav class="navigation posts-navigation" role="navigation">
		...
		<div class="nav-links">
			...
			<div class="nav-previous"><?php next_posts_link( __( 'Older posts', 'accesspress-store' ) ); ?></div>
			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next"><?php previous_posts_link( __( 'Newer posts', 'accesspress-store' ) ); ?></div>
			<?php endif; ?>
...
if ( ! function_exists( 'accesspress_store_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function accesspress_store_posted_on() {
...
if ( ! function_exists( 'accesspress_store_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function accesspress_store_entry_footer() {
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( __( 'Leave a comment', 'accesspress-store' ), __( '1 Comment', 'accesspress-store' ), __( '% Comments', 'accesspress-store' ) );
		echo '</span>';
	}
 ...
 * @return bool
 */
function accesspress_store_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'accesspress_store_categories' ) ) ) {
		...
		set_transient( 'accesspress_store_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so accesspress_store_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so accesspress_store_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in accesspress_store_categorized_blog.
 */
function accesspress_store_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'accesspress_store_categories' );
}
add_action( 'edit_category', 'accesspress_store_category_transient_flusher' );
add_action( 'save_post',     'accesspress_store_category_transient_flusher' );
