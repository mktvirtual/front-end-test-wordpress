<?php
	if(!class_exists('Accesspress_Store_Welcome')) :

		class Accesspress_Store_Welcome {
			...
			public function __construct() {
				...
				$this->companion_plugins = array(
					'woocommerce' => array(
						'slug' => 'woocommerce',
						'name' => esc_html__('Instant Demo Importer', 'accesspress-store'),
						'filename' =>'woocommerce.php',
						'class' => 'WooCommerce',
						'github_repo' => false,
						'bundled' => false,
						'info' => esc_html__('WooCommerce is a free eCommerce plugin that allows you to sell anything, beautifully.', 'accesspress-store'),
					),
				);
				...
				$this->req_plugins = array(
					...
					'instant-demo-importer' => array(
						...
						'info' => esc_html__('Instant Demo Importer Plugin adds the feature to Import the Demo Conent with a single click.', 'accesspress-store'),
					),
				...
				add_action( 'load-themes.php', array( $this, 'accesspressstore_activation_admin_notice' ) );

				/* Create a Welcome Page */
				add_action( 'admin_menu', array( $this, 'accesspressstore_welcome_register_menu' ) );

				/* Enqueue Styles & Scripts for Welcome Page */
				add_action( 'admin_enqueue_scripts', array( $this, 'accesspressstore_welcome_styles_and_scripts' ) );

				/** Plugin Installation Ajax **/
				add_action( 'wp_ajax_accesspressstore_plugin_installer', array( $this, 'accesspressstore_plugin_installer_callback' ) );

				/** Plugin Installation Ajax **/
				add_action( 'wp_ajax_accesspressstore_plugin_offline_installer', array( $this, 'accesspressstore_plugin_offline_installer_callback' ) );

				/** Plugin Activation Ajax **/
				add_action( 'wp_ajax_accesspressstore_plugin_activation', array( $this, 'accesspressstore_plugin_activation_callback' ) );

				/** Plugin Activation Ajax (Offline) **/
				add_action( 'wp_ajax_accesspressstore_plugin_offline_activation', array( $this, 'accesspressstore_plugin_offline_activation_callback' ) );
			...
			public function get_required_plugin_notification() {
				...
				$accesspressstore_plugin_installed_notif = get_option('accesspressstore_plugin_installed_notif');
				...
				update_option('accesspressstore_plugin_installed_notif', $notif_counter);
				return $notif_counter;
			}

			/** Welcome Message Notification on Theme Activation **/
			public function accesspressstore_activation_admin_notice() {
				global $pagenow;
			...
			public function accesspressstore_welcome_register_menu() {
				$action_count = get_option('accesspressstore_plugin_installed_notif');
				$title        = $action_count > 0 ? esc_html__( 'Welcome', 'accesspress-store' ) : esc_html__( 'Welcome', 'accesspress-store' );
				add_theme_page( 'Welcome', $title , 'edit_theme_options', 'accesspressstore-welcome', array( $this, 'accesspressstore_welcome_screen' ));
			}
			...
			public function accesspressstore_welcome_styles_and_scripts() {
				...
				wp_localize_script( 'accesspress-basic-welcome-screen', 'accesspressstoreWelcomeObject', array(
					'admin_nonce'	=> wp_create_nonce('accesspressstore_plugin_installer_nonce'),
					'activate_nonce'	=> wp_create_nonce('accesspressstore_plugin_activate_nonce'),
			...
			/** Plugin API **/
			public function accesspressstore_call_plugin_api( $plugin ) {
			...
			/** Check For Icon **/
			public function accesspressstore_check_for_icon( $arr ) {
				if ( ! empty( $arr['svg'] ) ) {
			...
			public function accesspressstore_plugin_active($plugin) {
				$folder_name = $plugin['slug'];
				$file_name = $plugin['filename'];
				$status = 'install';
			...
			public function accesspressstore_plugin_generate_url($status, $plugin) {
				$folder_name = $plugin['slug'];
				$file_name = $plugin['filename'];
			...
			/* ========== Plugin Installation Ajax =========== */
			public function accesspressstore_plugin_installer_callback(){
				...
				if (! wp_verify_nonce( $nonce, 'accesspressstore_plugin_installer_nonce' ))
					wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'accesspress-store') );
				...
				// Get Plugin Info
				$api = $this->accesspressstore_call_plugin_api($plugin);
			...
			/** Plugin Activation Ajax **/
			public function accesspressstore_plugin_activation_callback(){
				...
				if (! wp_verify_nonce( $nonce, 'accesspressstore_plugin_activate_nonce' ))
					die( esc_html__( 'Error - unable to verify nonce, please try again.', 'accesspress-store' ) );
				...
				// Get Plugin Info
				$api = $this->accesspressstore_call_plugin_api(esc_attr($plugin));
				...
				if($api->name){
					...
					$status = 'failed';
					$msg = esc_html__('There was an error activating $api->name', 'accesspress-store');
				}
			...
			public static function get_plugin_file( $plugin_slug ) {
	      	...
	      	public function get_local_dir_path($plugin) {
	      		...
	      		$url = wp_nonce_url(admin_url('themes.php?page=accesspressstore-welcome&section=import_demo'),'accesspressstore-file-installation');
	...
	if(class_exists('Instant_Demo_Importer')) :
		...
		$demoimporter->demos = array(
			'accesspress-store' => array(
				'title' => esc_html__('AccessPres Store Demo', 'accesspress-store'),
				'name' => 'accesspress-store',
				'screenshot' => get_template_directory_uri().'/welcome/demos/accesspress-store/screen.png',
				'home_page' => 'home',
				'menus' => array(
					'Test Menu' => 'primary'
				)
        ...
        //$demoimporter->set_customizer_options(get_template_directory().'/welcome/demos/accesspress-store/customizer_options.dat');
	endif;
?>