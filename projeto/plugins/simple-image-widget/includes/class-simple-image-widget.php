...
class Simple_Image_Widget extends WP_Widget {
	...
	public function __construct( $id_base = false, $name = false, $widget_options = array(), $control_options = array() ) {
		...
		$name    = ( $name ) ? $name : __( 'Imagem no Bloco', 'simple-image-widget' );

		$widget_options = wp_parse_args(
			$widget_options,
			array(
				'classname'   => 'widget_simpleimage', // Legacy class name.
				'description' => __( 'Inserir blocos em sua página', 'simple-image-widget' ),
			)
		);
	...
	public function form( $instance ) {
		...
		<div class="simple-image-widget-form">
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php _e( 'Titulo:', 'simple-image-widget' ); ?></label>
				<input type="text" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" value="<?php echo esc_attr( $instance['title'] ); ?>" class="widefat">
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'text' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'text' ) ); ?>"><?php _e( 'Distância do Titulo:', 'simple-image-widget' ); ?></label>
								<input name="<?php echo esc_attr( $this->get_field_name( 'text' ) ); ?>" value="<?php echo esc_textarea( $instance['text'] ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'text' ) ); ?>" class="widefat">
							</p>
			...
			<?php if ( ! is_simple_image_widget_legacy() ) : ?>
				<p class="simple-image-widget-control<?php echo ( $image_id ) ? ' has-image' : ''; ?>"
					data-title="<?php esc_attr_e( 'Carregar Imagem', 'simple-image-widget' ); ?>"
					data-update-text="<?php esc_attr_e( 'Atualizar Imagem', 'simple-image-widget' ); ?>"
					...
					<a href="#" class="<?php echo esc_attr( join( ' ', $button_class ) ); ?>"><?php _e( 'Carregar Imagem', 'simple-image-widget' ); ?></a>
				</p>
			<?php endif; ?>
					...
					switch ( $field ) {
						...
						case 'link' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'new_window' ) ); ?>" style="margin-top: -0.75em; padding-left: 2px">
								<label for="<?php echo esc_attr( $this->get_field_id( 'new_window' ) ); ?>">
									...
									<?php _e( 'Abrir em nova janela', 'simple-image-widget' ); ?>
								</label>
							</p>
							<?php
							break;
						...
						case 'link_text' :
							...
							<p class="<?php echo esc_attr( $this->siw_field_class( 'link_text' ) ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( 'link_text' ) ); ?>"><?php _e( 'Texto Botao', 'simple-image-widget' ); ?></label>
